...
/**
 * 嵌入视口描边：选区遮罩 + 视口悬停高亮，由 RenderEngine 帧钩子在 render 前刷新。
 */
export function createEmbedOutlineSync(deps: {
  ...
  let _cachedHoverMeshes: THREE.Mesh[] = []
  let _lastHoverVoxelKey = ''

  const hoveredVoxel = computed(() => {
    if (!deps.highlightOnHoverRef.value) return null
    return deps.hover.viewportBlock.value?.voxel ?? null
  })
    ...
    for (let z = 0; z < sizeZ; z++) {
      for (let row = 0; row < sizeRow; row++) {
        ...
        for (let col = 0; col < sizeCol; col++) {
          ...
          if (!entry || blockRegistryKeyForPalette(entry.registryId, entry.meta) !== blockId) continue
  ...
  function buildHoverMaskMeshes(hov: { column: number; row: number; zSlice: number }): THREE.Mesh[] {
    const def = deps.definitionRef.value
    if (!def) return []
    const { cellGrid, blockPalette } = def
    const sizeZ = cellGrid.length
  ...
  function flushHighlight(): void {
    ...
    const hoverKey = voxelKey(hov)
    if (hoverKey === _lastHoverVoxelKey) {
      deps.outlinePass.setMaskMeshes([..._selectionMasks, ..._cachedHoverMeshes])
      return
    }
    _lastHoverVoxelKey = hoverKey
    for (const m of _cachedHoverMeshes) {
      m.geometry?.dispose()
      ;(m.material as THREE.Material)?.dispose()
    }
    _cachedHoverMeshes = hov && deps.highlightOnHoverRef.value ? buildHoverMaskMeshes(hov) : []
    deps.outlinePass.setMaskMeshes([..._selectionMasks, ..._cachedHoverMeshes])
  }

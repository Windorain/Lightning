...
/** 生成 `{{渲染器|结构名称=…|…}}` wikitext（读 Main.embedPublish + initialCameras.embed） */
export function buildWikiEmbedWikitext(ctx: Context): string {
  ...
  const ic = ctx.main.initialCameras.embed.value
  ...
  if (ic.yawDeg != null) parts.push(`初始偏航角=${ic.yawDeg}`)
  if (ic.elevationDeg != null) parts.push(`初始俯仰角=${ic.elevationDeg}`)
  if (ic.zoom != null) parts.push(`初始缩放=${ic.zoom}`)
